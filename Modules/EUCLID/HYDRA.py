...
def processingCORE(module_data, coreZones, stadyStateTime, linesPerFigure, DtPlot, myOut, MyCore):
    ...
    for resultsGroup in module_data["data"] :
        ...
        if resultsGroup["dataType"] =="space": # значит имеем только один график от времени
            ...
            for zone in coreZones:
                ...
                processingSpaceType(resultsGroup, zone_name, object_name, dataList, lines_per_figure, timeStepPlot, module_name, zone["grid"], myOut, myData)
                MyCore.addDataToZone(zone_name, myData, "space")
